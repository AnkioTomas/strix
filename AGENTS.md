...
- **Open-source CLI (self-hosted):** free, fully local, BYO LLM key, needs Docker. Best for local dev loops, air-gapped/offline, and full control.
  ...
  export STRIX_REPORT_LANGUAGE=zh                   # report narrative language (zh/en/ja/Français/…); empty = unset
  # Or put STRIX_LLM / LLM_API_KEY / STRIX_REPORT_LANGUAGE in a CWD `.env` (shell export still wins)
