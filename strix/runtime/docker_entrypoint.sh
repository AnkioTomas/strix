...
# ``certutil -N`` on an already-initialized (or half-broken) NSS db can spin
# forever at ~100% CPU and block the entrypoint — containers never become
# ready and never exit cleanly. Always recreate the db, and bound each
# certutil call with ``timeout`` so a bad NSS never hangs the sandbox.
NSSDB="/home/pentester/.pki/nssdb"
sudo -u pentester rm -rf "$NSSDB"
sudo -u pentester mkdir -p "$NSSDB"
if sudo -u pentester timeout 20s certutil -N -d "sql:${NSSDB}" --empty-password; then
  if sudo -u pentester timeout 20s certutil -A -n "Testing Root CA" -t "C,," \
      -i /app/certs/ca.crt -d "sql:${NSSDB}"; then
    echo "✅ CA added to browser trust store"
  else
    echo "WARN: certutil -A failed or timed out; continuing without NSS trust"
  fi
else
  echo "WARN: certutil -N failed or timed out; continuing without NSS trust"
fi
